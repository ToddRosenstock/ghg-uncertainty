...
According to these distributions, the emission intensity at a particular per head milk yield can be described by a slice through the probability surface.
...
The herd structure interventions showed potential for causing major changes in milk yields and emission intensity on particular farms. Plotting these impacts against the initial sets of relevant herd attributes serves to illustrate which farms are particularly responsive to these interventions (Fig. S3).
...
```{r, fig.dim = c(12, 4), fig.cap="***Figure S13.*** *Strength of the relationship of Monte Carlo model input variables with the emission intensity of milk production, as indicated by the Variable-Importance-Score of a Partial Least Squares regression. This analysis is based on a*"}
