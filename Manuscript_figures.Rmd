...
```{r, warning=FALSE, message=FALSE,fig.dim = c(8, 4)}
...
all_farms_intensity<-ggplot(data=farm5_emissions$y, aes(CO2eq_per_milk)) +
geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666") +
...
dens1 + plot_spacer() + plot_spacer() + plot1 + dens2 + densi + 
...
```{r varkernelslicerange, message=FALSE, warning=FALSE, fig.dim = c(12, 4)}
...
- Fig. 6: Scatter plot that shows the impact of all interventions on per capita milk yield and emissions intensity
- Fig. 7: Scatter plot of all interventions against the backdrop of all samples drawn in second MC simulation (endline)
- Fig. 8: Intervention households shown on probability surface (Fig. 5)
- Fig. 9: Structural analysis - which household types have a chance of being detected
- Fig. 10: Errors (Type I and Type 2) - would be good to combine all interventions in one figure (might get crowded)
