...
all_farms_per_capita<-ggplot(data=GHG_simulation_scenarios$y, aes(pc_on_farm)) +
geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666") +
  ylab("Relative probability") + theme_bw() +
    xlab(expression(Total~greenhouse~gas~emissions~per~capita~(kg~CO[2]*'-eq'~kg^-1))) + ggtitle("Emissions per capita")

all_farms_per_capita

ggsave("figures/Fig_S1_emissions_from_all_farms_per_capita.png",width=9,height=5)


```

We can now look at summary statistics of total and per capita emissions from cattle across livestock-keeping smallholder farms in Kenya. To exclude farms that do not appear to be specializing in livestock, some of which feature extremely high (up to infinity in cases where no milk is produced) emissions intensity, we also look at summary statistics across only the farms with per capita milk yields of >500 kg of milk.
...
emissions_summary<-rbind(data.frame("Emissions per farm"=round(quantile(GHG_simulation_scenarios$y$on_farm,
                                    ...
                                    "Emissions per capita"=round(quantile(GHG_simulation_scenarios$y$pc_on_farm,
                                                                    c(0,0.05,0.5,0.95,1))/1000,1),
                         ...
                         data.frame("Emissions per farm"=round(quantile(dairy_households$y$on_farm,
                                                                  c(0,0.05,0.5,0.95,1))/1000,1),
                                    "Emissions per capita"=round(quantile(dairy_households$y$pc_on_farm,
                                                                    c(0,0.05,0.5,0.95,1))/1000,1),
...
column_spec(2:4, width = "5cm") %>% row_spec(0, align = "c") %>% 
  add_header_above(c(" " = 1, "Emissions per farm" = 1, "Emissions per capita" = 1, "Emissions intensity" = 1))
...
```{r, fig.dim = c(8, 4), fig.cap="***Figure 5.*** *Probability of a particular farm B featuring lower emissions intensity of milk production than another farm A, based on observed per-capita milk production levels. Estimates are derived from pairwise comparisons of slices through a population-wide generalization surface of the relationship between per-capita milk production and emissions intensity. This generalization was generated through Monte Carlo simulation, based on scenarios representing observed production settings on 414 livestock-keeping smallholder farms in Kenya.*"}
...
```{r, fig.dim = c(8, 8), fig.cap="***Figure S2.*** *Intervention impact on per capita milk production and greenhouse gas emissions intensity.*"}
...
```{r, fig.dim = c(8, 8), fig.cap="***Figure 6.*** *Intervention impacts on relative per capita milk production and relative greenhouse gas emissions intensity, compared to a counterfactual farm with similar properties that does not implement the respective intervention.*"}
...
metrics <- c("Milk yield per capita", "Emissions intensity of milk production")
...
kable(intervention_summary, booktabs = TRUE,digits=0,
  ...
  pack_rows(index = c("Impact on milk yield per capita (%)" = 5, "Impact on emissions intensity (%)" = 5), background="lightgray") %>%
...
```{r, fig.dim = c(8, 8), fig.cap="***Figure 7.*** *Relative intervention impacts on milk production per capita and greenhouse gas emissions intensity, in comparison to random changes between two evaluation times. The gray dots illustrate random changes that occurred on farms that made no management changes, whereas the colored dots show changes on farms that implemented the respective interventions.*"}
...
kable(intervention_end_summary, booktabs = TRUE,digits=0,
  ...
  pack_rows(index = c("Impact on milk yield per capita (%)" = 6, "Impact on emissions intensity (%)" = 6), background="lightgray") %>%
column_spec(2:6, width = "3cm")
