...
# create work station
soil<- read.csv("data.csv")
soil<- dplyr::select(soil,soc,depth,bdensity,cfragments)
input_table<- read.csv("MC simulation data.csv")
input_table<-dplyr::select(input_table,-median)
...
```{r, echo= FALSE}
knitr::kable(input_table, digits = 2, caption = "Monte Carlo input data")
```
