...
We're defining the function as in 210617_GHG_model.Rmd, but there's no need to show this again here.
...
The new function will consist of a wrapper around the mcSimulation function, which will be able to modify the input table according to a scenario file we provide. This file will contain information on which variables should be modified for each scenario and how many model runs should be included in the simulation.

```{r scenario_sampler}
scenario_sampler<-function(base_estimate, scenarios, model_function, ..., numberOfModelRuns=NA, randomMethod = "calculate", functionSyntax="data.frameNames",
  relativeTolerance = 0.05,
  verbosity = 0)
{
  initial_estimate = estimate_read_csv(base_estimate)
  scenarios<-read.csv(scenarios)
  
  scenario_names<-colnames(scenarios[3:ncol(scenarios)])
  
  for(scens in scenario_names)
  {
    estim<-initial_estimate
    for (i in 2:nrow(scenarios))
    ...
    if("Runs" %in% scenarios[1,])
      n_Runs<-as.numeric(scenarios[which(scenarios[1,]=="Runs"),scens]) else
        if(!is.na(numberOfModelRuns))
          n_Runs<-numberOfModelRuns else
...
```{r}
knitr::kable(read.csv("data/scenarios.csv"))
...
GHG_simulation<-scenario_sampler(base_estimate = "data/livestock_ghg_input_table.csv",
...
```{r}
GHG_simulation_PLS<-GHG_simulation
GHG_simulation_PLS$x<-GHG_simulation_PLS$x[,1:ncol(GHG_simulation_PLS$x)-1]
GHG_simulation_PLS$y<-GHG_simulation_PLS$y[,1:ncol(GHG_simulation_PLS$y)-1]

pls_result <- plsr.mcSimulation(object = GHG_simulation_PLS,
                  resultName = names(GHG_simulation_PLS$y)[7], ncomp = 1)
```
