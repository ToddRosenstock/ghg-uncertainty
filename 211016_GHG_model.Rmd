            ...
            D-->S(Sold); linkStyle 3 stroke: red, stroke-width:1px
                S-->L(Liquid storage); linkStyle 4 stroke: red, stroke-width:1px
                    ...
                    L-->Bi(Biogas); linkStyle 8 stroke: green, stroke-width:1px
                        Bi-->Ls(Liquid); linkStyle 9 stroke: red, stroke-width:1px
        ")
```
...
```{r make_variables, warning=FALSE}
...
#mms_liquid<-liquid*(1-share_biogas)
...
if(weight_gain_WG==0)
  N_retention<-((milk_yield*(1.9+0.4*milk_fat)/100)/6.38) else
...
As of version 1.107 of `decisionSupport`, the package contains the `scenario_mc` function, which allows specifying different distributions of the input variables for specific subsets of the total number of model runs. We'll use this functionality here to simulate results for actual herd compositions encountered in Kenya.

```{r scenario_making}
Kenya_herds<-read.csv("data/Kenya_baseline_individual_cow_wNotes.csv")

herds<-table(Kenya_herds[,c("QQNO","animal_type")])

mms<-t(aggregate(Kenya_herds[,c("mms_pasture","mms_spread","mms_drylot","mms_solidstore","mms_composted","mms_liquid","mms_biogas","mms_burn","system","feed_kgCO2","feed_transport_kgCO2")],by=list(Kenya_herds$QQNO), FUN=median))
mms<-mms[2:nrow(mms),]
rownames(mms)[which(rownames(mms)=="system")]<-"feeding_system"
rownames(mms)[which(rownames(mms)=="feed_kgCO2")]<-"feed_prod_CO2"
rownames(mms)[which(rownames(mms)=="feed_transport_kgCO2")]<-"feed_trans_CO2"

herd_scenarios<-cbind(data.frame(Variable=paste0("N_animal_type_",1:12),param=rep("both",12)),
                      as.data.frame.matrix(t(herds)))
colnames(herd_scenarios)[3:ncol(herd_scenarios)]<-paste0("farm_",1:(ncol(herd_scenarios)-2))

mms_scenarios<-cbind(data.frame(Variable=rownames(mms),param=rep("both",nrow(mms))),
                     as.data.frame.matrix(mms))
colnames(mms_scenarios)[3:ncol(mms_scenarios)]<-paste0("farm_",1:(ncol(mms_scenarios)-2))

scenarios<-rbind(herd_scenarios,mms_scenarios)
...
With these scenarios, we can now feed the Monte Carlo simulation with realistic farm compositions. To adequately consider uncertainty, we can run a user-specified number of random scenarios for each farm.

```{r scenario_runs, warning=FALSE}

GHG_simulation_real_farms<-scenario_mc(base_estimate=estimate_read_csv("data/livestock_ghg_input_table.csv"),
                     scenarios=scenarios,
                     model_function = ghg_emissions,
...
enteric_CH4_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "enteric_CH4",
                   method = "boxplot_density",
                   old_names = "enteric_CH4",
                   new_names = "Total Methane Emissions from Livestock by Enteric Fermentation") + 
...
milk_yield_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "milk_yield",
                   method = "boxplot_density",
                   old_names = "milk_yield",
                   new_names = "Total Milk Yield") + 
...
enteric_CH4_CO2eq_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "enteric_CH4_CO2eq",
                   method = "boxplot_density",
                   old_names = "enteric_CH4_CO2eq",
                   new_names = "Carbon Dioxide Equivalent Enteric Methane Emissions from Livestock") + 
...
mm_CH4_CO2eq_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "mm_CH4_CO2eq",
                   method = "boxplot_density",
                   old_names = "mm_CH4_CO2eq",
                   new_names = "Carbon Dioxide Equivalent Methane Emissions from Livestock Manure") + 
...
mm_N2O_CO2eq_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "mm_N2O_CO2eq",
                   method = "boxplot_density",
                   old_names = "mm_N2O_CO2eq",
                   new_names = "Carbon Dioxide Equivalent Nitrous Oxide Emissions from Livestock Manure") + 
...
feed_CO2_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "feed_CO2",
                   method = "boxplot_density",
                   old_names = "feed_CO2",
                   new_names = "Carbon Dioxide Emissions from Livestock Feed Production and Transport") + 
...
on_farm_dist_plot_farms <- plot_distributions(mcSimulation_object = GHG_simulation_real_farms,
                   vars = "on_farm",
                   method = "boxplot_density",
                   old_names = "on_farm",
                   new_names = "Total Carbon Dioxide Equivalent Emissions from Livestock") + 
...
Here's the plot that results from using the same parameters for all farms:
...
And here's what happens when we run 10 random scenarios for each real herd composition:

```{r}
enteric_CH4_dist_plot_farms + milk_yield_dist_plot_farms + 
  enteric_CH4_CO2eq_dist_plot_farms + mm_CH4_CO2eq_dist_plot_farms + 
  mm_N2O_CO2eq_dist_plot_farms + feed_CO2_dist_plot_farms + 
  on_farm_dist_plot_farms + 
plot_layout(guides = "collect", design = layout)
...
```{r}
pls_input<-GHG_simulation_real_farms
pls_input$x<-pls_input$x[,which(!("Scenario" == colnames(pls_input$x)))]
pls_result <- plsr.mcSimulation(object = pls_input,
                  resultName = names(pls_input$y)[7], ncomp = 1)
```

We run the `plot_pls()` on the results from `plsr.mcSimulation()` with a number of standard settings. The length of the bars is equal to VIP with a vertical line at '1' on the x-axis indicating a standard cut-off for VIP used for variable selection [@whitney_simulating_2017]. The overall plot only shows those variables with a VIP > 0.8, which is the common threshold for variable selection [@lanzanovaImprovingDevelopmentEfficiency2019b, @luedeling2016decision]. The colors of the bars represent the positive or negative coefficient of the given input variable with the output variable.
...
We use the `varkernelslicerange()` function to show the milk yield (influencing variable `in_var` value) intervals. Here we choose a very wide distribution (the whole range of milk yields from the Monte Carlo simulation). Here we set the maximum expected value for milk yield `in_var` to `r round(max(in_var), digits = 0)` and the minimum expected value to `r round(min(in_var), digits = 0)`.
