...
# We're not sure what the animal types stand for, but this is important for calculating emissions.
# We need to know, which types are dairy cattle, which ones are lactating cows, which ones are bulls, and which ones are 'others'. We have 12 animal types.
  
animal_types_lact_cow<-c(4,5,6) # lactating cows - important to note here that the original data table appears to have worked with the actual number of months that cows were pregnant. This resulted in a weighted mean of the various maintenance coefficients. I'm not sure where we could get such data.
...
# animal type 5 - cows
# animal type 6 - cows
# animal type 7 - females <= 12 months
# animal type 8 - males <= 12 months
# animal type 9 - females 12-36 months
# animal type 10 - female calves (<12 months)
# animal type 11 - male calves (<12 months)
...
# Ratio of net energy available for growth in a diet to digestable energy consumed (Equation 10.15)
...
enteric_CH4_CO2eq<-enteric_CH4*25
...
mm_N2O_CO2eq<-(mm_direct_n2o+mm_vol_n2o+mm_leach_n2o)*298 # (298 is the CO2-equivalent of N2O)
...
return(list(enteric_CH4=enteric_CH4,
            ...
            enteric_CH4_CO2eq=enteric_CH4_CO2eq,
            mm_CH4_CO2eq=mm_CH4_CO2eq,
            mm_N2O_CO2eq=mm_N2O_CO2eq,
            feed_CO2=feed_CO2,
            on_farm=on_farm))
}
...
uncertainty::varscatter(in_var = GHG_simulation$x$milk_yield, out_var = GHG_simulation$y$enteric_CH4_CO2eq)
```
