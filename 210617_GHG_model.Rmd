...
The formulas come from Chapter 10 of the *2006 IPCC Guidelines for National Greenhouse Gas Inventories* [@dong_ipcc_2006]. The document can be found on [the IPCC website](https://www.ipcc-nggip.iges.or.jp/public/2006gl/pdf/4_Volume4/V4_10_Ch10_Livestock.pdf). The chapter is part of the book *Good Practice Guidance and Uncertainty Management in National Greenhouse Gas Inventories* [@ipcc_good_2021] , also on the [the IPCC website](https://www.ipcc-nggip.iges.or.jp/public/gp/english/). Many variables are based on the work done in *Variation in the carbon footprint of milk production on smallholder dairy farms in central Kenya* [@wilkes_variation_2020] and in *Methods and guidance to support MRV of livestock emissions* [@wilkes_methods_2019].
...
```{r make_variables}
###CW Note ####
### We will need to clean up the warnings later
...
### ####
n_intake<-(gross_energy_intake/18.45)*((perc_crude_protein_diet/100)/6.25) # equation 10.32

# There was some code here on N retention in milk. That's apparently from equation 10.33, but it only had the first part of the equation, missing the part on weight gain. We decided to go with the alternative way of estimating N retention (figures given in Table 10.20)
# n_retention<-((milk_yield*(3.3/100))/6.38)

# Table 10.20 contains N retention coefficients:
# Dairy cows: 0.20 N_retention_dairy_cow
# Other cattle: 0.07 N_retention_other_cattle

animal_type_dairy_cow=1 ###ASSUMPTION

if(animal_type==animal_type_dairy_cow)
  n_excretion<-n_intake*(1-N_retention_dairy_cow)*365 else
    n_excretion<-n_intake*(1-N_retention_other_cattle)*365 #equation 10.31

# Direct nitrous oxide emissions (Eq 10.25 from IPCC guidelines)
...
# Nitrous oxide (N2O) from volatization (Eq 10.26 from IPCC guidelines)
mm_vol_n2o<-n_excretion*((mms_pasture*mnvc_pasture)+
...
mm_CO2_N2Od<-mm_direct_n2o*298*365 # (298 is the CO2-equivalent of N2O)
