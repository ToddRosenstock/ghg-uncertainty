...
```{r}
scenarios<-read.table("data/scenarios.csv", sep = ",", fileEncoding = "UTF-8-BOM",header=TRUE)
...
variables_to_update<-c(paste0("N_animal_type_",1:12),"feeding_system")

scenarios<-data.frame(Variables=variables_to_update,param="both")
...
write.csv(scenarios,"data/farm_scenarios.csv")
...
GHG_simulation_scenarios<-scenario_mc(base_estimate = estimate_read_csv("data/livestock_ghg_input_table.csv"),
                                 scenarios = read.csv("data/farm_scenarios.csv",fileEncoding="UTF-8-BOM")[1:2,],
