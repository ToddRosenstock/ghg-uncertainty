...
# I suspect this is because of an error in the varslice_resample function (and also the varkernelslice functions).
# I suspect the problem is in   Relative_probability <- in_outkernel$z[, expectedin_var]
# There may not be an expectedin_var'th column - this should be the column that corresponds to the x value expectedin_var

while (more_milk < max(in_var))
{
  more_milk <- more_milk + 10
  slice_more_milk <-
    varslice_resample(in_var,
                      out_var,
                      expectedin_var = more_milk,
                      n_runs = 100000)
  results <-
    rbind(results, c(more_milk, sum(sliceb > slice_more_milk) / length(sliceb)))
}
