...
ggplot(data=slices, aes(Values, fill=Level)) +
 geom_density(alpha=.2) +
  ylab("Relative probability") +
    xlab(expression(Emission~intensity~of~milk~production~at~specific~milk~yield~per~capita~(kg~CO[2]*'-eq'~kg^-1))) + labs(fill='Production level') 
...
plot_nobull <- ggplot(plotdata_breed, aes(x = in_var, y = out_var)) + 
  geom_point(shape = 20, color = "blue", size = 2) + 
  stat_smooth(method = "loess", fullrange = TRUE) +
  geom_density_2d() +
  geom_rug(color="lightskyblue3") + 
...
dens_nobull<-ggplot(plotdata_breed, aes(x = in_var, y = out_var)) +
  stat_density_2d(
    geom = "raster",
    aes(fill = after_stat(density)),
    contour = FALSE
...
ggsave("Fig_13_no_bulls_scatter_per_head_milk_emissions_intensity.png",width=9,height=9)
...
ggsave("Fig_14_nobull_reductions.png", width=9,height=9)
...
ggsave("Fig_15_lessReplace_reductions.png", width=9,height=9)
