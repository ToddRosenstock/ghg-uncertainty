...
GHG_simulation_scenarios$y<-GHG_simulation_scenarios$y[which(GHG_simulation_scenarios$y$pc_milk_yield>500),]

#GHG_simulation_scenarios$y<-GHG_simulation_scenarios$y[is.finite(GHG_simulation_scenarios$y$CO2eq_per_milk),]

#GHG_simulation_scenarios$y[GHG_simulation_scenarios$y[,"CO2eq_per_milk"]>10,]

#GHG_simulation_scenarios$y
```
...
# THIS LOOKS BAD, BECAUSE OF Low-Milk-high-emissions households. Would be good to break the axis - any ideas? (I removed the extreme peak now by excluding underperforming dairy households)
...
ggplot(data=slice$slice, aes(CO2eq_per_milk)) +
geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666") +
  ylab("Relative probability") +
    xlab(expression(Emission~density~of~milk~production~(XX~CO[2]-eq~kg^-1)))
...
# THIS WON'T REALLY WORK... peak is too high, everything else becomes invisible (should work now, afte excluding underperforming households)
